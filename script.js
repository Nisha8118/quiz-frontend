...
// ---- DOM refs (declared FIRST so auth callback can use them safely) ----
...
async function loadQuestion() {
  ...
  show(submitBtn);
  hide(nextBtn);
  if (feedback) { feedback.textContent = ""; feedback.className = "status"; }
  ...
  try {
    ...
    if (questionCounter) questionCounter.textContent = `Question ${answered + 1} / ${totalTarget}`;
...
submitBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus(
      feedback,
      data.correct
        ? `✅ Correct! ${data.explanation || ""} ${insight}`
        : `❌ Wrong. Correct: ${data.correct_answer} ${data.explanation || ""} ${insight}`,
      data.correct ? "success" : "error"
    );
    if (difficultyNote) appendStatus(feedback, difficultyNote, data.correct ? "success" : "info");
...
function finish() {
  ...
  const weakPreview = weakQuestions.slice(0, 3).map(q => `• ${q}`).join("<br>");
  const strongPreview = strongQuestions.slice(0, 3).map(q => `• ${q}`).join("<br>");
  if (weakTopics) weakTopics.innerHTML = `<h4>❌ Weak Areas</h4>${weakPreview || "None"}`;
  if (strongTopics) strongTopics.innerHTML = `<h4>✅ Strong Areas</h4>${strongPreview || "None"}`;

  if (recommendationText) {
    if (pct >= 80) recommendationText.textContent = "Excellent performance. Move to HARD difficulty and advanced topics.";
    else if (pct >= 60) recommendationText.textContent = "Good performance. Revise the weak areas shown above and retry.";
    else recommendationText.textContent = "Focus on the weak areas identified by AI before attempting higher difficulty questions.";
  }
  ...
  history.unshift({
    subject: mode === "subject" ? (customSubj.value.trim() || subjectSel?.value || "") : "PDF Quiz",
    score,
    answered,
    accuracy: pct,
...
function drawHistoryChart() {
  ...
  historyChart = new Chart(ctx, {
    ...
    options: {
      responsive: true,
      scales: {
        y: { min: 0, max: 100, ticks: { color: "#ffffff" } },
